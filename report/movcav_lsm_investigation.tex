...
\normalsize \textbf{I.}~OIFS step-0 coastline crash \emph{(resolved)} \quad\textbullet\quad \textbf{II.}~FESOM cavity-margin vertical-CFL instability \emph{(root cause: a geostrophically-unbalanced remapped state --- restart-value fixes tested \& excluded --- \S\ref{sec:rev3}, read first)}}
\author{Experiment \texttt{movcav4}, chunk-3 (awiesm3 1901)}
\date{2026-07-05}

\begin{document}

\begin{center}\fcolorbox{red}{red!8}{\parbox{0.93\textwidth}{%
\textbf{\textcolor{red}{BLOCKER II ROOT CAUSE REFUTED (2026-07-13).}} Blocker~I (OIFS coastline
crash) stands. \textbf{Blocker~II's stated root cause --- ``a geostrophically-unbalanced remapped
state'', cured by a thermal-wind velocity init --- is FALSIFIED.} That fix was implemented and
failed its own offline gate. A \textbf{cold start on the very same PISM cavity, at the same 1200\,s
timestep, is stable}, so neither the geometry nor the balance argument is the problem; the killer is
the \emph{dynamics seam} between the bit-exactly preserved evolved state and the patched changed
nodes. Read \texttt{report/dynamics\_seam\_2026-07-13}, then
\texttt{report/increment\_test\_2026-07-13}. The evolution/step-1 analysis and figures below remain
valid as \emph{observations}; the \emph{causal claim} does not.}}\end{center}
...
\smallskip\noindent
\textbf{Blocker II --- a FESOM vertical-CFL blowup --- is the LIVE blocker, now root-caused.}
Some $5$--$10$ model days in, FESOM blows up with a vertical-CFL runaway ringing the cavity
margin. The cause is a \textbf{geostrophically-unbalanced remapped state}: the mesh change plants
fresh cavity fronts against salty open water but \emph{zeroes} the balancing current, so the
model's $t=0$ adjustment grows $w$ at the fronts. This is the \emph{same} instability the hourly
movie showed \emph{growing over days} --- a $t=0$ imbalance that amplifies, not a melt process
from a balanced start. Every restart-\emph{value} fix (freezing cap; coherent $T/S$ refill to
zero density inversions) was tested and does \textbf{not} prevent it; a 60\,s step clears the
\emph{fast} mode but a \emph{slow} barotropic \code{eta\_n} mode then grows. Fix: initialise the
changed-cell velocity in balance. \textbf{\S\ref{sec:rev3} is the current truth --- read it first.}
\end{abstract}

\section{Current state (2026-07-06, rev.~3): the blowup is a \emph{dynamical imbalance} of the remapped state, not a restart-value pathology}\label{sec:rev3}

\textbf{Read this first --- supersedes \S\ref{sec:current} (rev.~2).} The rev.~2 basal-melt /
thin-layer story led to two concrete restart fixes --- a freezing-point cap, then a full
coherent-column $T/S$ refill. Both are now \textbf{implemented and tested} on a clean leg
(\code{movcav8}, awiesm3 1901); together they make the remapped restart \textbf{provably clean}
(zero static density inversions anywhere) --- yet \textbf{neither prevents the blowup}. The true
root is that the remap hands FESOM a state that is \textbf{out of geostrophic balance at the
moving margins}: real (and fill-sharpened) lateral density fronts carried across the mesh change,
with the balancing current \textbf{zeroed out}. That single imbalance expresses \emph{fast} as
the vertical-CFL runaway (1200\,s) and \emph{slow} as a barotropic \code{eta\_n} runaway (60\,s).

\subsection{What was tested since rev.~2, and what it falsified}
\begin{enumerate}[leftmargin=1.6em,itemsep=3pt]
  \item \textbf{Freezing-point cap --- applied, no effect.} \code{cap\_new\_cavity\_temp} ran in
        \code{movcav8}'s \code{couple\_in} (1968 nodes / 14\,670 cells capped to $T_f$). The leg
        still blew (\code{mstep}$\approx$205, \code{CFLz}$\approx$5.7). Removing the thermal
        driving does \emph{not} stop it.
  \item \textbf{Coherent-column $T/S$ refill --- all inversions removed, still blows.} The
        per-level unmasked nearest-node fill (which planted melt-fresh $S\approx22$ mid-column)
        was replaced by (a) a \emph{mask-aware open-ocean donor} for surface-opened columns and
        (b) an \emph{own shelf-base hold} for still-sub-ice retreated columns (committed,
        \code{ad4e84b2}). Over all 1595 retreated cells the worst static stability improved
        $-8.8\!\to\!-0.5$\,kg\,m$^{-3}$ (11 severe $\to$ \textbf{0}). The leg \emph{still} blew
        (\code{mstep}$\approx$164, \code{CFLz}$\approx$8.5), essentially unchanged.
        \textbf{Restart $T/S$/\code{ssh}/density is decisively excluded} (four inversion-free
        variants all blew the same way).
  \item \textbf{Timestep $=60$\,s --- clears the fast blowup, exposes a slow one.} With
        \code{step\_per\_day=1440} (everything else identical) the run made \textbf{0} \code{CFLz}
        warnings and reached \code{mstep}$=14\,629$ ($\approx$10 model-days, 4$\times$ longer),
        then died on \textbf{\code{eta\_n}$\to$NaN} (barotropic ssh) at 112.8/$-$65.9. So the
        \emph{fast} blowup is genuinely vertical-CFL (timestep-curable) but a \emph{slow
        barotropic} mode remains --- the $\Delta t$ ramp alone is \textbf{not} a cure.
\end{enumerate}

\subsection{The initiation mechanism (fast blowup): unbalanced fronts $+$ zero velocity}
The updated plots (\code{movcav8} v4 restart; PolyCollection on the real mesh triangles,
dateline-wrapping triangles filtered, layer-index vertical axis so the thin cavity layers are
visible; \file{movcav4\_crash\_plots/initstate/}, Figs.~\ref{fig:initstate},~\ref{fig:hetero})
show it directly. Around the \emph{earliest} \code{CFLz} seed (Weddell, $-62.96/-66.81$, first
warning \code{mstep}~4, peak level~11 $\approx95$\,m):
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  \item \textbf{A picket-fence of fresh cavity columns.} Opened cells (ice retreated) come out
        \textbf{uniformly fresh, $S\approx31$}, punched into an $S\approx33$--$34$ background ---
        strong \textbf{lateral density fronts at the grid scale ($\sim$25\,km)}, strongest
        mid-column. Advanced cells (open$\to$new ice) keep salty $\sim$34 water and are
        comparatively benign --- so the \emph{opened} cells are the troublemakers.
  \item \textbf{Velocity is exactly zero across the whole opened region} --- the remap zeroes
        \code{u}/\code{v} on every changed element; the speed panels are solid black except the
        far-field unchanged edge.
  \item A density front \emph{with zero velocity} is \textbf{not in thermal-wind balance}: at
        $t=0$ the model must spin up the missing geostrophic current, and its \textbf{ageostrophic
        part is $w$}. The $w$ panels show exactly that --- a down/upwelling dipole \textbf{pinned
        to the fronts}, mid-deep layers. At $\Delta t=1200$\,s over $\sim$10\,m layers this $w$
        ($\sim$2\,mm\,s$^{-1}$) gives \textbf{\code{CFLz}$>1$ within $\sim$4--10 steps}.
\end{itemize}
This is precisely why the $T/S$ \emph{stability} fixes did nothing: the imbalance is
\textbf{lateral (fronts) $+$ missing momentum}, not a vertical inversion. The
\emph{over-freshening} is partly self-inflicted --- the shelf-base ``hold upward'' plants the
freshest (ice-base) water throughout the newly-opened column, sharpening the very fronts that
drive the adjustment.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{movcav4_crash_plots/initstate/planview_seed_weddell.png}\\[2pt]
  \includegraphics[width=\linewidth]{movcav4_crash_plots/initstate/section_lat_seed_weddell.png}
  \caption{\textbf{Initiation seed --- Weddell margin ($-62.96/-66.81$), \code{movcav8} v4
    restart.} \emph{Top:} plan view at $\sim$95\,m (level~11). Note the isolated \textbf{fresh
    column ($S\approx31$)} at the ice front (black outline), the \textbf{all-black speed panel}
    (\code{u}/\code{v}$=0$ across the opened region), and the $w$ dipole on the front. Grey $=$
    cavity/deep (no data at this level, \emph{not} land). \emph{Bottom:} lat--depth section on a
    \textbf{layer-index axis} (equal layers; depth on the right) --- the fresh column runs
    full-depth under a thin hatched ice cap, against $S\approx34$ open water, with the $w$
    down/upwelling curtain at the front. The state has strong fronts but \emph{zero} balancing
    flow.}
  \label{fig:initstate}
\end{figure}

\subsection{Peak \code{CFLz} is a consequence, not the seed}
The \emph{highest}-\code{CFLz} cell at the crash (90.5/$-$63.5, level~38, \textbf{3775\,m},
\code{CFLz}~8.45) is \textbf{deep, fully-unchanged open ocean with no cavity at all}, and a
perfectly smooth initial state ($S$ varies 0.02\,PSU; Fig.~\ref{fig:hetero}, top). It is a
\emph{late} manifestation: the instability seeds at the opened margins and
\textbf{advects/propagates into the deep basin by the crash step}. So ``high \code{CFLz} at the
crash'' $\ne$ ``where it started'' --- only the earliest (margin) cells show the initiating
imbalance.

\subsection{The slow (60\,s) blowup is the same root, a different mode}
At 60\,s the vertical CFL stays subcritical, so instead a \textbf{barotropic \code{eta\_n}
response accumulates over $\sim$10 days} and piles against the coast/slope \emph{adjacent} to the
changed margins. The 60\,s crash site (112.8/$-$65.9, Fig.~\ref{fig:hetero}, bottom) is a
well-stratified continental slope (warm CDW at depth, surface jet) --- a \emph{benign} local
state, but only 32\,km from the advanced-ice margin (9 changed cells within 100\,km). So
\code{eta\_n}$\to$NaN there is a \textbf{slow remote pile-up}, not a local shock. Fast
(\code{CFLz}) and slow (\code{eta\_n}) are \textbf{two faces of one imbalance}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{movcav4_crash_plots/initstate/planview_A_90E_L38.png}\\[2pt]
  \includegraphics[width=\linewidth]{movcav4_crash_plots/initstate/planview_D_60s_etacrash.png}
  \caption{\textbf{Two other high-\code{CFLz} sites --- both benign at $t=0$, confirming the seed
    is elsewhere.} \emph{Top:} the peak-\code{CFLz} cell (90.5/$-$63.5, level~38, \textbf{3775\,m})
    is \textbf{fully-unchanged deep open ocean} ($S$ varies 0.02\,PSU) --- a late consequence of
    the instability spreading, not a cause. \emph{Bottom:} the \textbf{60\,s \code{eta\_n} crash
    site} (112.8/$-$65.9) is unchanged coastal slope 32\,km from the advanced-ice margin --- a
    mild state where the barotropic mode piles up over $\sim$10 days.}
  \label{fig:hetero}
\end{figure}

\subsection{Reconciling the two views: a $t=0$ imbalance that \emph{amplifies}}
The rev.~2 hourly movie (\code{movcav4}; Figs.~\ref{fig:mapevo},~\ref{fig:sectevo}) and this
step-1 analysis (\code{movcav8}) are the \emph{same} instability at two times; they reconcile
once ``clean'' is read correctly:
\begin{itemize}[leftmargin=1.4em,itemsep=1pt]
  \item \textbf{``Hour~1 clean'' meant clean \emph{values}, not a balanced state.} $T/S$/\code{ssh}
        are in normal ranges with no NaNs, but the state is out of geostrophic balance (fresh
        margin fronts vs salty open water, \code{u}/\code{v}$=0$). The movie's ``$w\approx0$ at
        hour~1'' \emph{is} that zero initial velocity.
  \item \textbf{The day-scale ``growth'' is the adjustment of the $t=0$ imbalance}, not a new
        process --- the missing geostrophic current spins up, $w$ amplifies at the fronts, the
        fresh/cold band saturates and spreads: exactly the movie's onset$\to$runaway.
  \item \textbf{Melt is a secondary amplifier, not the driver.} rev.~2 pinned the freshening on
        melt under new ice (\emph{advanced} cells); the fresh fronts are largely a remap-fill
        feature at the \emph{opened} cells, present at $t=0$, and the freezing cap (which removes
        the melt driving) does not stop the blowup.
\end{itemize}
So rev.~2 read the \emph{character} right (a fresh-fronted $\sim$95\,m coastal baroclinic mode
that grows over days) but mis-read normal restart \emph{values} as a balanced IC and over-credited
melt. The unifying cause is the unbalanced remapped state.

\subsection{Root cause and ranked fix}
On the spun-up full mesh these fronts carry their balancing geostrophic currents; the remap
preserves $T/S$ (density) but \textbf{discards the momentum field on changed cells (zeroes
\code{u}/\code{v}) and over-sharpens the opened columns}, handing the model a state far from
balance. It must then adjust --- fast (vertical CFL) or slow (barotropic). Ranked:
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt]
  \item \textbf{Initialise \code{u}/\code{v} in thermal-wind balance with the remapped density}
        (or copy the neighbouring unchanged current onto changed elements) instead of zeroing.
        Removes the root; addresses \emph{both} modes. \textbf{Highest leverage.}
  \item \textbf{Do not over-fresh the opened columns} --- blend toward the adjacent open-ocean
        profile to soften the grid-scale fronts.
  \item \textbf{$\Delta t$ ramp} for the mesh-change leg --- necessary but \emph{not sufficient}
        (60\,s cleared the fast mode but the slow barotropic one still killed it at day~10); must
        be combined with (1)/(2).
\end{enumerate}
Everything observed --- fronts $+$ black speed panels $+$ $w$ dipoles on the fronts, fast and
slow --- points to (1): give the changed cells a \emph{balanced} velocity instead of zero.

\section{Current state (2026-07-05, rev.~2, \emph{superseded by \S\ref{sec:rev3}}): a \emph{time-growing} coastal baroclinic instability on the submesh --- \emph{narrowed, not yet root-caused}}\label{sec:current}
...
\begin{enumerate}[leftmargin=1.6em,itemsep=3pt]
  ...
  \item \textbf{Timestep / vertical-CFL artifact --- \emph{revised by the 60\,s test
        (\S\ref{sec:rev3}): the fast blowup \underline{is} vertical-CFL; 600\,s was simply not
        small enough}.} Halving to \textbf{600\,s} (\code{step\_per\_day=144}, run
        \code{26061443}) still blew (\code{mstep}$=845\approx5.9$\,d vs the 1200\,s baseline's
        $4.3$\,d) with \code{CFLz} still $61$--$71$ at depth --- which \emph{looked} like ``not
        $\Delta t$''. That inference was wrong: the adjustment $w$ is still supercritical at
        600\,s, so the runaway regenerates. Going 20$\times$ smaller settles it ---
        \textbf{at 60\,s (\code{step\_per\_day=1440}) the leg makes \emph{zero} \code{CFLz}
        warnings and runs $\approx$10 model-days (\code{mstep}$=14\,629$)}, confirming the
        \emph{fast} mode is a genuine vertical-CFL instability of the unbalanced initial state.
        It is \textbf{not} a full cure, though: at 60\,s a \emph{slow} barotropic \code{eta\_n}
        mode instead grows over $\sim$10 days and NaNs (\S\ref{sec:rev3}). So $\Delta t$ is a
        necessary lever for the fast mode but must be paired with fixing the initial imbalance.
...
\subsection{rev.~2 hypothesis: a basal-melt shock at newly-under-ice nodes (tested, superseded by \S\ref{sec:rev3})}\label{sec:rootcause}
The rev.~2 chain, each step measured (verify-on-output plus a diagnostic re-run with hourly
\code{fw}/\code{fh}/\code{N2}/\code{Kv}, run \code{26063909}): the mesh change puts new ice over
above-freezing open-ocean water ($\mathrm{ul}_{\text{new}}>\mathrm{ul}_{\text{old}}$ at
\textbf{1878} nodes, 667 open$\to$cavity, ringing the margin, drafts to $\sim$490\,m); the remap
copies that warm water straight under the ice; melt fires immediately (\code{fh}
$\sim$250$\to$2000\,W/m$^2$ band on/along the crash front; \code{fw}$\approx$100\,m/yr,
$100\times$ physical); the freshwater dumped into the $\sim$10\,m (min 4.6\,m) front layer gives
$\Delta S=-4$ to $-8$\,PSU/12\,h, top-down at the ice base (Fig.~\ref{fig:sectevo}) $\to$ buoyant
overturning $\to$ CFLz. \textbf{This was implemented and tested and does \emph{not} hold as the
driver:} the freezing cap \code{cap\_new\_cavity\_temp} (called at the end of
\code{remap\_all\_restarts}; drives the front $T-T_f$ to exactly $0$ on the 1878 nodes)
\emph{and} a full coherent $T/S$ refill were both run in \code{movcav8}, and \textbf{neither
prevents the blowup} (\S\ref{sec:rev3}). Melt is a \emph{secondary} amplifier of a state already
unbalanced at $t=0$ (reconciliation, \S\ref{sec:rev3}). \emph{Structural aside, still worth a
check:} deep-draft open$\to$cavity transitions ($130$--$490$\,m of ice in one 10-yr PISM step)
are physically implausible --- verify the submesh \code{cavity\_nlvls} against the PISM ice field.
...
\begin{figure}[htbp]
  ...
  \caption{\textbf{Cross-margin vertical section} at lon\,$\approx$\,108.73$^\circ$E
    ...
    This is the clearest single view of the instability \emph{amplifying in time} from a
    value-clean but dynamically-unbalanced start (\S\ref{sec:rev3}). Frames from
    \file{blowup\_section\_evolution.mp4}.}
  \label{fig:sectevo}
\end{figure}
