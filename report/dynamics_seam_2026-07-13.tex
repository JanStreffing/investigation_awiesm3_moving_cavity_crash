...
\textbf{\textcolor{red}{PARTIALLY REFUTED (2026-07-13, evening).}} The core diagnosis in this note
stands: the cavity \emph{geometry} is fine, a cold start on it is stable, and the killer is the
remapped \emph{dynamics seam}. \textbf{But its central premise about the FIX is wrong.} This note
argues the per-leg PISM increment is ``two orders of magnitude smaller'' (mean
$|\Delta\mathrm{draft}| \approx 22$\,m) and therefore absorbable. It is not. The increment test was
run: \textbf{1399 nodes have their ice removed ENTIRELY in one leg}, the Ross front retreats
$\approx$440~km, and PISM loses 24\% of its floating area per decade. The mean is diluted by the
98\% unchanged interior and is \textbf{meaningless}. Struck-out passages below are the refuted ones.
See \texttt{report/increment\_test\_2026-07-13}.}}\end{center}
...
\midrule
\sout{\textbf{a genuine 10-yr PISM increment}: mean $|\Delta \mathrm{draft}|$} & \sout{\textbf{$\approx$22\,m}} \textcolor{red}{$\leftarrow$ \textbf{REFUTED: the mean is meaningless; 1399 nodes lose their ice entirely, max 923\,m}} \\
\bottomrule
\end{tabular}
\end{center}
...
it.} \sout{What production must survive, forever, is the \emph{incremental} change from one PISM
chunk to the next --- two orders of magnitude smaller ---} and \textbf{that has never been
tested}.
\textcolor{red}{\textbf{REFUTED:} the increment is \emph{not} ``two orders of magnitude smaller''.
PISM is in disequilibrium and sheds its shelves: 1399 nodes lose their ice outright in a single leg
(Ross front $-$440\,km, floating area $-$24\%/decade). The strike-through above is the error.}
...
\item \textbf{Then test the change that matters.} By chunk~3 the cold-started ocean carries a
      ...
      10-yr PISM increment \sout{($\approx$22\,m)} \textcolor{red}{(in fact: 1399 nodes fully opened)}. \textbf{This is the experiment that decides whether
...
\item \textbf{If even \sout{$\approx$22\,m} \textcolor{red}{a real increment} dies}, the seam is fundamental and the remaining option is
