...
inventoried and instrumented, reproduction pending (\S\ref{sec:voskin-inventory}).
\end{abstract}
...
\code{CFL\_z} all normal, and \code{[FLUXGUARD]} stayed \textbf{silent}. The one field that exploded is
the SSH-solver right-hand-side: \code{ssh\_rhs}$=7.1\times10^{6}$ (from $-208$ the previous step). So
this is the \emph{external-mode (SSH) equation} (\code{compute\_ssh\_rhs\_ale}, \code{oce\_ale.F90})
blowing up at an ice-shelf node on step~1. Crucially, this run is already on the safe free-surface
scheme --- \code{which\_ale='linfs'} (\code{step\_per\_day=72}, i.e.\ a 1200\,s step), the scheme
\S\ref{sec:linfs-coupled} showed \emph{preserves} the cavity where \code{zstar} collapses it. So face
(iii) is \textbf{not} the \code{zstar} seam collapse and \code{linfs} does not cure it: the couple-in /
cavity update is handing FESOM a transport--geometry mismatch that detonates the SSH solve on step~1,
independently of the ALE choice --- \emph{not} advection, \emph{not} \code{hnode} (that seam,
\S\ref{sec:hnode}, was fixed; \code{hnode} is clean here), and \emph{not} the ALE-mode issue of
\S\ref{sec:linfs-coupled}. This is a genuinely new face: an SSH-RHS explosion seeded at the open-cavity
couple-in under \code{linfs}. The daily stream captured nothing
(crash before day~1 closed) and the guard correctly stayed silent, validating the instrumentation while
telling us this crash is not flux-driven.
