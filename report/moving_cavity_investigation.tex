...
\author{}
\date{Last updated: 2026-07-18 (evening)}
...
PI-control cavity melt runs $\sim7\times$ observations. That melt is traced not to the melt
scheme or the ocean temperature but to cavity currents $\sim6\times$ too fast; the plan is a
separate, tunable ice-base drag coefficient (\S\ref{sec:hygiene}, \S\ref{sec:melt2}).
\end{abstract}
...
\item \textbf{The high, rising cavity melt is diagnosed} (\S\ref{sec:melt2}): near-realistic
  ...
  cavity currents $\sim6\times$ too fast (median 13\,cm/s). Fix planned: a separate tunable
  ice-base drag \code{C\_d\_cavity}. Open.
