...
\medskip\noindent
Remaining open items: the increment test itself --- the chunk-3 leg on the changed mesh dies
after weeks of model time from \textbf{phantom zero-water ingestion at draft-changed cavity
columns} (mechanism proven, fix candidates identified, \S\ref{sec:zerowater}) --- and the
too-weak Southern-Hemisphere summer sea-ice melt-back relative to the 500-year v3.4 reference
(\S\ref{sec:state}).
\end{abstract}
...
%======================================================================
\section{The increment test crashes: phantom zero-water at changed cavity columns
  (2026-07-17)}\label{sec:zerowater}
...
($\eta$ jumping $\pm 40$\,m in one step at the Antarctic Peninsula tip; day 19--51 depending on
MPI reduction order --- the state is metastable). The autopsy was made cheap by two facts: the
leg costs 3--4 minutes of wall time, and a temporary XIOS file group with
\code{sync\_freq="1d"} flushes daily 2-D/3-D fields to disk as they complete, so even a
crashed run leaves full evidence (the default XIOS files sync only on close: 51 days of
output, zero records).

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{zero_water_george6.png}
\caption{The chunk-3 killer, shown in one basin (George VI / Marguerite Bay; FESOM elements
rendered individually, black outlines = the 73 elements whose cavity draft the mesh change
touched). (a) What the mesh change did: $\pm$1--6 cavity levels along the George VI sound and
the Marguerite Bay margin. (b) By day 3 the uppermost-wet-level salinity has dropped up to
25\,psu \emph{exactly on the outlined elements} --- the injection is the changed-column set.
(c) By day 46 the phantom freshwater has flooded the basin and is exporting along the coast.
(d) The proof: $(S,T)$ at the ice-base level of four injector columns slides along the mixing
lines (dashed) toward exactly $(0,0)$ --- T- and S-replacement fractions are identical, which
only a $(T,S)=(0,0)$ end-member produces; trajectories pin at FESOM's $S=3$ clip with
$T\!\to\!0$. The same pattern repeats at every fringing shelf around the continent
(Getz, Victoria Land, Shackleton, West); basin-mean symptoms are the ring freshening and the
coastal SSH pile-up with \emph{negative} surface freshwater flux --- the ``flood'' carries no
water.}
\label{fig:zerowater}
\end{figure}

\textbf{Mechanism.} Cavity columns have a pinned free surface ($\eta \equiv 0$, layer
thicknesses frozen), so nothing relaxes a flow imbalance there. The remap leaves a small
divergence at draft-changed columns that drives vertical velocity through the ice base
(seeded at $10^{-4}$\,m\,s$^{-1}$, exactly the observed early stripping rate; it grows
$100\times$ through a feedback --- stripping makes the column buoyant, buoyancy drives more
$w$ --- visible as 78\,704 CFL$_z$ warnings in 51 days vs.\ 125 per \emph{year} in chunk-1,
every hotspot within $0.1^\circ$ of a changed column). FESOM's vertical tracer machinery
treats the top interface like a \emph{free surface}, where entering water carries zero tracer
(correct for rain; \code{oce\_adv\_tra\_ver.F90}, top-layer coefficients) --- under an ice
shelf the same treatment strips $T$ and $S$ toward $(0,0)$ for as long as $w$ persists.
Chunk-1 is immune because balanced cavities have $w(\mathrm{base})\!\approx\!0$; the flaw is
armed only by a mesh change.

\textbf{Ruled out by measurement} (each was a plausible suspect): surface freshwater fluxes
(\code{fw} is $50\times$ too small at the injectors, and \emph{negative} over the ring);
OIFS runoff/calving and the \code{ECE\_LANDICE} snow cap (mapper config identical between
chunks, remap weight sums $\le 1$, ICMGG \code{sd} hygiene clean); the OASIS \code{feom} masks
(identical, all-unmasked, both chunks); the remapped restart state ($T/S$/\code{hnode}
consistent to the level, freezing cap ran: 1137 columns); the mesh files (zero elem/node
level-range violations); the zero-filled dry restart levels themselves (padding them with
nearest-wet values --- now a permanent remap pass --- reproduced the crash bitwise: the zero
end-member is the boundary treatment, not the restart). The couple\_in warning about a missing
\code{cap\_new\_cavity\_temp.py} is a red herring: the freezing cap runs as Fortran inside the
remap binary; the warning refers to a Python duplicate that never existed.

\textbf{Fix candidates} (open): (i) remap-side --- start draft-changed cavity columns at rest
(zero $u,v,w$ and the AB arrays at flagged columns) so the seed pump never forms and the
columns adjust by gravity waves; one 4-minute test. (ii) FESOM-side, the deeper fix --- a
cavity-aware top boundary in the vertical tracer advection: under \code{ulevels>1} the
top-interface exchange must carry the local concentration (material interface), not zero.
Needs care to keep FCT conservation intact.

\section{State of the cycle and of the climate (2026-07-17)}\label{sec:state}
