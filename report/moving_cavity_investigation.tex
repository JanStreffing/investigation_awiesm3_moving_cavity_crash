...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{zero_water_mechanism.png}
\caption{The chunk-3 killer. (a) Ice-base salinity drop over the first two days (reds) against
the cavity columns whose draft levels the mesh change touched (dark gray): the injection ring
is the changed-column set. (b) The proof: $(S,T)$ at the ice-base level of five injector
columns slides along the mixing lines (dashed) toward exactly $(0,0)$ --- T- and S-replacement
fractions are identical, which only a $(T,S)=(0,0)$ end-member produces; trajectories pin at
FESOM's $S=3$ clip and $T\!\to\!0$. (c,d) Downstream symptoms: the whole ring south of
58$^\circ$S freshens monotonically from day 1 while coastal SSH climbs out of the chunk-1
stability band (gray), with \emph{negative} surface freshwater flux --- the ``flood'' carries
no water.}
\label{fig:zerowater}
\end{figure}
